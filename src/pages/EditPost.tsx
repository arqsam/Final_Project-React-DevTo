...
export default function EditPost() {
  ...
  const { register, handleSubmit, setValue } = useForm<CreatePostData>();
  ...
  async function onSubmit(data: CreatePostData) {
    ...
    if (res?.message) {
      ...
      alert("Error onSubmit Edit Post");
    }
  }
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/posts/${id ?? 0}`)
      ...
      .catch((error) => "ERROR IN EDIT POST");
  }, [id, setValue]);
    ...
    <>
      <nav className="container grid grid-cols-12 h-auto mx-auto  w-screen mt-5">
        <div className="flex items-center w-full justify-between col-span-12">
          ...
          <p className="mx-5 w-40 font-semibold"> Edit Post </p>
                ...
                <div className="flex flex-row w-auto p-3">
                  {
                    <input
                      ...
                      onFocus={() => setTitleFocused(true)}
                      className="text-center outline-none border-none h-10 placeholder:text-black/40 placeholder:text-sm"
